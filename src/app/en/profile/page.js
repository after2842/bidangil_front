    ...
    <div className="min-h-screen  text-white bg-gray-950 px-4 py-6">
      ...
      <div className="max-w-3xl mx-auto mb-6">
        <h2 className="text-2xl font-semibold font-myfont text-center">
          {userNickname && "Hello, " + userNickname}
        </h2>
      </div>

      {/* In-Progress Orders */}
      <div className="max-w-4xl mx-auto mb-12 mt-10">
        <h3 className="text-3xl font-bold mb-0 font-myfont">
          Orders in Progress
        </h3>
      ...
      <div className="max-w-3xl mx-auto">
        <h3 className="text-xl font-bold mb-4 font-myfont">Order History</h3>
        {pastData && pastData.length === 0 ? (
          <p className="text-gray-300">No past orders.</p>
        ) : (
          pastData &&
          pastData.map((order) => (
            <div
            ...
            >
              <div>
                ...
                <p className="text-sm text-gray-400">
                  Order Date: {LocalTime(order["order_created_at"])}
                </p>{" "}
                <p className="text-sm text-gray-400">
                  Delivery Date: {LocalTime(order.Delivery["delivered_at"])}
                </p>
              </div>
              <div className="text-sm font-semibold text-blue-500">
                {order?.Payment?.total_price != null
                  ? "$" + order.Payment.total_price.toLocaleString()
                  : ""}
              </div>
            </div>
          ))
              ...
              <div className="relative flex flex-1 flex-col justify-between gap-3">
                <div className="space-y-1 md:p-4">
                  {profileData && profileData.length === 0 ? (
                    <p className="text-gray-950">No orders in progress.</p>
                  ) : (
                    <div
                      key={profileData.id}
                ...
                {showPayment ? (
                  <div>
                    <div className="md:text-sm text-black">
                      Order Date: {LocalTime(profileData.order_created_at)}{" "}
                      <br />
                    </div>
                    <button
                  ...
                  <div className="md:text-sm text-black">
                    Order Date: {LocalTime(profileData.order_created_at)} <br />
                  </div>
                )}
              </div>
            </div>
